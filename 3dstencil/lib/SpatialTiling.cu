...
__global__ void SpatialTiling_Kernel(VALUE_T* __restrict gm_input, VALUE_T* __restrict gm_output, int NZ, int NY, int NX){
	extern __shared__ char sm[];
	constexpr int RAD = KernelInfo<KERNEL_T>::RAD;
	constexpr int threads_z_num = DefaultParams::threads_z_num;
	constexpr int threads_y_num = DefaultParams::threads_y_num;
	constexpr int threads_x_num = DefaultParams::threads_x_num;
	constexpr int CTX_SZ = threads_z_num * threads_y_num * threads_x_num;
	constexpr int gm_z_vaild_tile = threads_z_num;
	constexpr int gm_y_vaild_tile = threads_y_num;
	constexpr int gm_x_vaild_tile = threads_x_num;
	constexpr int gm_z_halo_tile = gm_z_vaild_tile + RAD * 2;
	constexpr int gm_y_halo_tile = gm_y_vaild_tile + RAD * 2;
	constexpr int gm_x_halo_tile = gm_x_vaild_tile + RAD * 2;
	constexpr int sm_z_tile = gm_z_halo_tile;
	constexpr int sm_y_tile = gm_y_halo_tile;
	constexpr int sm_x_tile = gm_x_halo_tile;

	const int thread_idx = threads_y_num * threads_x_num * threadIdx.z + threads_x_num * threadIdx.y + threadIdx.x;
	const int gm_z_vaild_begin = blockIdx.z * gm_z_vaild_tile;
	const int gm_z_vaild_end = min(gm_z_vaild_begin + gm_z_vaild_tile, NZ);
	const int gm_z_halo_begin = gm_z_vaild_begin - RAD;
	const int gm_z_halo_end = gm_z_vaild_end + RAD;
	...
	for(int i = thread_idx; i < sm_z_tile * sm_y_tile * sm_x_tile; i += CTX_SZ){
		int data_z = i / (sm_x_tile * sm_y_tile);
		int sm_z = data_z;
		int gm_z = min(max(gm_z_halo_begin + data_z, 0), NZ - 1);
		int data_y = (i / sm_x_tile) % sm_y_tile;
		int sm_y = data_y;
		int gm_y = min(max(gm_y_halo_begin + data_y, 0), NY - 1);
		int data_x = i % sm_x_tile;
		int sm_x = data_x;
		int gm_x = min(max(gm_x_halo_begin + data_x, 0), NX - 1);
		sm_data[Offset_sm(sm_z, sm_y, sm_x)] = gm_input[Offset_gm(gm_z, gm_y, gm_x)];
	}
	__syncthreads();
	#pragma unroll
	for(int data_z = 0; data_z < RAD * 2 + 1; data_z++){
		int sm_z = data_z + threadIdx.z;
		#pragma unroll
		for(int data_y = 0; data_y < RAD * 2 + 1; data_y++){
			...
			for(int data_x = 0; data_x < RAD * 2 + 1; data_x++){
				...
				reg_input[data_z][data_y][data_x] = sm_data[Offset_sm(sm_z, sm_y, sm_x)];
			}
		}
	}
	utils::kernelCompute<KERNEL_T, VALUE_T>(&reg_output, reg_input);
	{
		int gm_z = gm_z_vaild_begin + threadIdx.z;
		int gm_y = gm_y_vaild_begin + threadIdx.y;
		int gm_x = gm_x_vaild_begin + threadIdx.x;
		if(gm_z < gm_z_vaild_end && gm_y < gm_y_vaild_end && gm_x < gm_x_vaild_end){
			gm_output[Offset_gm(gm_z, gm_y, gm_x)] = reg_output;
		}
	}
}
...
void SpatialTiling(VALUE_T *init_data, VALUE_T *output_data, int NZ, int NY, int NX, int T, bool enable_warmup){
	...
	const int grid_dim_z = (NZ + gm_z_vaild_tile - 1) / gm_z_vaild_tile;
	const int grid_dim_y = (NY + gm_y_vaild_tile - 1) / gm_y_vaild_tile;
	const int grid_dim_x = (NX + gm_x_vaild_tile - 1) / gm_x_vaild_tile;
	const size_t sm_size = sizeof(VALUE_T) * (gm_z_halo_tile * gm_y_halo_tile * gm_x_halo_tile);
	...
	LOG_INFO("grid_dim_x: %d, grid_dim_y: %d, grid_dim_z: %d, CTX_SZ: %d, sm_size: %lu\n", grid_dim_x, grid_dim_y, grid_dim_z, CTX_SZ, sm_size);

	dim3 gridDim(grid_dim_x, grid_dim_y, grid_dim_z), blockDim(threads_x_num, threads_y_num, threads_z_num);
	if(enable_warmup){
		cudaEventRecord(start);
		kernel_func<<<gridDim, blockDim, sm_size>>>(gm_input, gm_output, NZ, NY, NX);
		...
		LOG_INFO("Warmup iterations: %d (estimated for ~5s)\n", warmup_iterations);
	...
	LOG_CRITICAL("SpatialTiling: %lf GStencil/s, %lf TFLOPS\n", gstencils, tflops);
...
template void SpatialTiling<KernelType::j3d7pt, double>(double *init_data, double *output_data, int NZ, int NY, int NX, int T, bool enable_warmup);
template void SpatialTiling<KernelType::j3d13pt, double>(double *init_data, double *output_data, int NZ, int NY, int NX, int T, bool enable_warmup);
template void SpatialTiling<KernelType::poisson, double>(double *init_data, double *output_data, int NZ, int NY, int NX, int T, bool enable_warmup);
template void SpatialTiling<KernelType::j3d27pt, double>(double *init_data, double *output_data, int NZ, int NY, int NX, int T, bool enable_warmup);
